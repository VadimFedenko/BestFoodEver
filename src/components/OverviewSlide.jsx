    ...
    <div className="space-y-3 sm:space-y-4">
      {hasUnavailableIngredients && (
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-2.5 sm:p-4">
          <div className="flex items-center gap-2 text-blue-600 dark:text-blue-300 mb-1 sm:mb-2">
            ...
            <span className="text-xs sm:text-sm font-semibold">Cannot be prepared in your region</span>
          </div>
          <div className="text-xs sm:text-sm text-surface-600 dark:text-surface-300">
            {unavailableIngredients.map((ing, idx) => (
              <span key={idx} className="font-mono">{ing.displayName ?? ing.name} ({ing.grams}g){idx < unavailableIngredients.length - 1 ? ', ' : ''}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {hasDataWarnings && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-2.5 sm:p-4">
          <div className="flex items-center gap-2 text-amber-600 dark:text-amber-300 mb-1 sm:mb-2">
            ...
            <span className="text-xs sm:text-sm font-semibold">Data warnings</span>
          </div>
          <div className="text-xs sm:text-sm text-surface-600 dark:text-surface-300">
            {missingIngredients?.length > 0 && <div>Missing: {missingIngredients.join(', ')}</div>}
            {missingPrices?.length > 0 && <div>No costs: {missingPrices.join(', ')}</div>}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        <div className="bg-surface-100/80 dark:bg-surface-800/80 rounded-lg p-2 sm:p-4 text-center">
          ...
          <div className="text-[10px] sm:text-xs text-surface-500">Time</div>
        ...
        <div className="bg-surface-100/80 dark:bg-surface-800/80 rounded-lg p-2 sm:p-4 text-center">
          ...
          <div className="text-[10px] sm:text-xs text-surface-500">Cost{priceUnitLabel}</div>
        ...
        <div className="bg-surface-100/80 dark:bg-surface-800/80 rounded-lg p-2 sm:p-4 text-center">
          ...
          <div className="text-[10px] sm:text-xs text-surface-500">Calories</div>
      ...
      {ingredients?.length > 0 && (
        <div className="bg-surface-100/80 dark:bg-surface-800/80 rounded-lg p-2.5 sm:p-4">
          <h4 className="text-xs sm:text-sm font-semibold text-surface-700 dark:text-surface-200 mb-1.5 sm:mb-3">Ingredients</h4>
